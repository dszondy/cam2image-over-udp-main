...
namespace image_tools_sdfr
{
  ...
  private:
    ...
    void initialize()
    {
      ...
      {
        process_image_brightness(container, this->get_logger());
      };
    ...
    IMAGE_TOOLS_SDFR_LOCAL
    void process_image_brightness(
        const image_tools_sdfr::ROSCvMatContainer &container, rclcpp::Logger logger)
    {
      ...
      {
        cv::Mat temp_frame;
        container.is_bigendian() ? cv::cvtColor(frame, temp_frame, cv::COLOR_YUV2GRAY_UYVY) : cv::cvtColor(frame, temp_frame, cv::COLOR_YUV2GRAY_YUYV);
      ...
      bool light = average_brightness > 50.0;
